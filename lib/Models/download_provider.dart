import 'dart:async';
import 'package:flutter/foundation.dart'; // [QUAN TRỌNG] Import để dùng kIsWeb
...
class DownloadingProvider extends ChangeNotifier {
  ...
  // [SỬA 1] Bọc kiểm tra Web cho getter isPaused
  // Nếu là Web hoặc chưa có tên store, luôn trả về false
  bool get isPaused {
    if (kIsWeb || _storeName == null) return false;
    return FMTCStore(_storeName!).download.isPaused();
  }
  ...
  Future<void> assignDownload({
    ...
    // [SỬA 2] Nếu là Web, chặn ngay lập tức không cho chạy logic download
    if (kIsWeb) {
      debugPrint("Web does not support downloading features.");
      return Future.value();
    }
  ...
  Future<void> pause() async {
    // [SỬA 3] Chặn Web
    if (kIsWeb || _storeName == null) return;
  ...
  Future<void> cancel() {
    // [SỬA 5] Chặn Web
    if (kIsWeb || _storeName == null) return Future.value();
